...
function Settings() {
    ...
    try {
      if (profile_pic) {
        ...
        const res = await axios.post(
          import.meta.env.VITE_BACKEND_ENDPOINT + "/user_profile_pic",
          formData,
          {
            headers: {
  ...
  const handleFormSubmit1 = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await axios.post(
        import.meta.env.VITE_BACKEND_ENDPOINT + "/user_pref", json,
        {
          headers: {
            "Access-Control-Allow-Origin":"*",
            Authorization: authHeader(),
  ...
  return (
    <div>
      ...
      <form id='form1' onSubmit={handleFormSubmit1}>
        <label>
          Race:
          <input type="text" value={race} onChange={handleRaceChange} onInput={handleRaceChange} onFocus={handleRaceChange} onBlur={handleRaceChange} />
        </label>
        <br />
        <label>
          Religion:
          <input type="text" value={religion} onChange={handleReligionChange} onInput={handleReligionChange} onFocus={handleReligionChange} onBlur={handleReligionChange} />
        </label>
        <br />
        <label>
          Gender:
          <input type="text" value={gender} onChange={handleGenderChange} onInput={handleGenderChange} onFocus={handleGenderChange} onBlur={handleGenderChange} />
        </label>
        <br />
      ...
      <form id='form2' onSubmit={handleFormSubmit2}>
        <label>
          ...
          {isEditing ? (
            <div>
              <input type="file" accept=".jpg,.jpeg,.png" onChange={(e) => setProfile_pic(e.target.files ? e.target.files[0] : null)} />
